...
contract("MultiSigWallet", (accounts) => {
  ...
  beforeEach(async () => {
    ...
    submitOwnerForm = await contractInstance.submitOwnerForm(
      web3.utils.asciiToHex("add owner"),
      3,
      saeed,
      false,
      { from: alice }
    ...
    submitRemovedOwnerForm = await contractInstance.submitOwnerForm(
      web3.utils.asciiToHex("removed owner"),
      2,
      jack,
      true,
      { from: bob }
  ...
  it("only submit RemovedOwnerForm with available owner addresses", async () => {
    await utils.shouldThrow(
      contractInstance.submitOwnerForm(2, saeed, true, { from: bob })
    );
  });
  ...
  it("Revoke Confirmed OwnerForm and RemovedOwnerForm", async () => {
    ...
    await utils.shouldThrow(
      contractInstance.revokeOwnerForm(formId, { from: jack })
    );
    await utils.shouldThrow(
      contractInstance.revokeOwnerForm(removedFormId, { from: alice })
    );
  ...
  it("Excute OwnerForm", async () => {
    ...
    await utils.shouldThrow(
      contractInstance.excuteOwnerForm(formId, { from: alice })
    );
  ...
  it("Excute RemovedOwnerForm", async () => {
    ...
    await utils.shouldThrow(
      contractInstance.excuteOwnerForm(removedFormId, { from: alice })
    );
    ...
    const removedOwnerForm = await contractInstance.getOwnerForm(
      removedFormId,
      { from: alice }
    );
